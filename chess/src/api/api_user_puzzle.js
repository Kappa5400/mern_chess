...
export const getPuzzleByPuzzleID = async (puzzleID) => {
  const res = await fetch(`${BASE_URL}/v1/userpuzzle/Bypuzzleid/${puzzleID}`, {
    method: "GET",
    headers: getHeaders(),
  });
...
export const createUserPuzzle = async ({ fen, answer, rating }, token) => {
  const res = await fetch(`${BASE_URL}/v1/userpuzzle/createuserpuzzle`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const updateUserPuzzle = async (
  ...
  const res = await fetch(
    `${BASE_URL}/v1/userpuzzle/updatepuzzle/${puzzleID}`,
    {
      method: "PATCH",
      headers: getHeaders(),
...
export const deleteUserPuzzle = async (puzzleID) => {
  const res = await fetch(`${BASE_URL}/v1/userpuzzle/delete/${puzzleID}`, {
    method: "DELETE",
    headers: getHeaders(),
  });
